...
async function getServerInfo (dataReq) {
    let collection = sysDB.collection('accounts');
    const docs = await collection.find({'token': dataReq.token}, {'databases': 1}).toArray();
...
module.exports = {
    ...
    //     }
    // },
    getDocuments: async (serverInfo, collectionName, query, options) => {
        try {
    ...
    getDocumentsv2: async (dataReq, collectionName, query, options) => {
        try {
